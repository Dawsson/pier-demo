...
export function CounterValue({ value }: { readonly value: number }) {
  ...
  return (
    <span
      ...
      aria-live="polite"
      className="flex min-w-[2ch] justify-center px-3 py-2 font-semibold text-8xl leading-none tabular-nums sm:text-[7.5rem] md:text-[8.25rem]"
    >
      {characters.map((character, index) => {
        return (
          <span
            aria-hidden
            className="relative inline-grid h-[1.08em] w-[0.64em] place-items-center overflow-hidden"
            key={index}
          >
